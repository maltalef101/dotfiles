...
function M.setup()
	local config = {
		virtual_text = mvim.lsp.diagnostics.virtual_text,
		signs = mvim.lsp.diagnostics.signs,
		underline = mvim.lsp.diagnostics.underline,
		update_in_insert = mvim.lsp.diagnostics.update_in_insert,
		severity_sort = mvim.lsp.diagnostics.severity_sort,
	}
	if vim.fn.has("nvim-0.5.1") > 0 then
		vim.lsp.handlers["textDocument/publishDiagnostics"] = function(_, result, ctx, _)
			...
			vim.lsp.diagnostics.save(diagnostics, bufnr, ctx.client.id)
		...
		vim.lsp.handlers["textDocument/publishDiagnostics"] = function(_, _, params, client_id, _)
			...
			vim.lsp.diagnostics.save(diagnostics, bufnr, client_id)
			if not vim.api.nvim_buf_is_loaded(bufnr) then
				return
			end
  ...
  vim.lsp.handlers["textDocument/publishDiagnostics"] = vim.lsp.with(vim.lsp.diagnostic.on_publish_diagnostics, {
    virtual_text = mvim.lsp.diagnostics.virtual_text,
    signs = mvim.lsp.diagnostics.signs.active,
    underline = mvim.lsp.document_highlight,
  })

  vim.lsp.handlers["textDocument/hover"] = vim.lsp.with(vim.lsp.handlers.hover, {
    border = mvim.lsp.popup_border,
  })
...
function M.show_line_diagnostics()
	...
	local width = 0
	local opts = {}
	local close_events = { "CursorMoved", "CursorMoved1", "BufHidden", "InsertCharPre" }
	...
	for i, diagnostic in ipairs(diagnostics) do
		...
		vim.lapi.nvim_win_set_option(winnr, "winblend", 0)
		vim.lapi.nvim_buf_set_var(bufnr, "lsp_floating_window", winnr)
