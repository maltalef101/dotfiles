...
local function preview_location_callback(result)
	...
	if vim.tbl_islist(result) then
		...
		M.floating_buf, M.flaoting_win = create_floating_file(result[1], opts)
	else
		M.prev_result = result
		M.floating_buf, M.flaoting_win = create_floating_file(result, opts)
	end
end
...
local function preview_location_callback_new_signature(_, _, result)
	return preview_location_callback(result)
end
...
function M.Peek(what)
	if vim.tbl_contains(vim.api.nvim_list_wins(), M.floating_win) then
		local success_1, _ = pcall(vim.api.nvim_set_current_win, M.flaoting_win)
		...
		if not success then
			print('peek: Error calling LSP method "textDocument/' .. what .. '". The current language server lsp might not support it.')
		end
	end
end
